...
// The film is 60% of the bundle and the only thing in it that is not audio or a
// zip, so it needs its own read-it-the-way-the-consumer-will check. An MP4 is a
// tree of length-prefixed boxes: walking the top level end to end proves the
// lengths are self-consistent, which is what damage of the kind this package has
// already seen destroys. A truncated or byte-shifted file runs off the end or
// lands on a box with a nonsense size, and both are caught here.
func TestBundledFilmIsAWholeMP4(t *testing.T) {
	found := 0
	forEachBundled(t, ".mp4", func(name string, body []byte) {
		found++
		boxes, err := walkMP4(body)
		if err != nil {
			t.Errorf("%s: %v", name, err)
			return
		}
		// ftyp says it is an MP4 at all; moov is the index, without which a
		// player has nothing; mdat is the media itself. A file missing any of
		// the three is not a film.
		for _, want := range []string{"ftyp", "moov", "mdat"} {
			if !boxes[want] {
				t.Errorf("%s: no %s box", name, want)
			}
		}
		// faststart, which the encode asks for: with moov ahead of mdat a
		// browser can start playing before the whole file has arrived, and
		// SoundStorm streams these over a network by definition.
		if boxes["mdat-before-moov"] {
			t.Errorf("%s: moov comes after mdat, so playback cannot start until "+
				"the whole file has downloaded (encode with -movflags +faststart)", name)
		}
	})
	if found == 0 {
		t.Fatal("no film in the bundle")
	}
}

func TestMP4WalkNoticesATruncatedFile(t *testing.T) {
	var whole []byte
	forEachBundled(t, ".mp4", func(_ string, body []byte) {
		if whole == nil {
			whole = body
		}
	})
	if whole == nil {
		t.Skip("no film to damage")
	}
	if _, err := walkMP4(whole); err != nil {
		t.Fatalf("the undamaged film did not pass: %v", err)
	}
	// Cut one byte off the end. Every box header still reads correctly; the last
	// one simply no longer contains what it says it does, which is exactly the
	// shape of the corruption this file is guarding against.
	if _, err := walkMP4(whole[:len(whole)-1]); err == nil {
		t.Error("a byte was removed and the walk did not notice")
	}
}

// walkMP4 walks the top-level box list and reports which box types it saw.
//
// The key "mdat-before-moov" is set when the media data comes first, which is
// the layout that prevents progressive playback.
func walkMP4(b []byte) (map[string]bool, error) {
	seen := map[string]bool{}
	sawMdat := false

	for i := 0; i < len(b); {
		if i+8 > len(b) {
			return seen, fmt.Errorf("%d trailing byte(s) at %d: not a box header", len(b)-i, i)
		}
		size := int64(b[i])<<24 | int64(b[i+1])<<16 | int64(b[i+2])<<8 | int64(b[i+3])
		...
		case 1:
			...
			if i+16 > len(b) {
				return seen, fmt.Errorf("box %q at %d claims a 64-bit size and has no room for one", kind, i)
			}
			size = 0
			for _, x := range b[i+8 : i+16] {
				size = size<<8 | int64(x)
			}
			header = 16
		}
		...
		if size < header {
			return seen, fmt.Errorf("box %q at %d has size %d, smaller than its own header", kind, i, size)
		}
		if int64(i)+size > int64(len(b)) {
			return seen, fmt.Errorf("box %q at %d runs %d byte(s) past the end of the file",
				kind, i, int64(i)+size-int64(len(b)))
		}
		for _, c := range kind {
			if c < 0x20 || c > 0x7e {
				return seen, fmt.Errorf("box at %d has an unprintable type %q", i, kind)
			}
		}
		...
		if kind == "moov" && sawMdat {
			seen["mdat-before-moov"] = true
		}
		i += int(size)
	}
	return seen, nil
